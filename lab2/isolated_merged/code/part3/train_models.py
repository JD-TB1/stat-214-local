...
def evaluate_primary_splits(
    ...
    for split_id in get_primary_by_image_split_ids(split_root=split_root):
        print(f"[part3] evaluating split {split_id}")
        ...
        for block_name in FEATURE_BLOCKS:
            ...
            for family, grid in model_grids.items():
                print(f"[part3] tuning {split_id} :: {family} :: {block_name} ({len(grid)} configs)")
                ...
                print(
                    "[part3] selected "
                    f"{split_id} :: {family} :: {block_name} :: "
                    f"val_roc_auc={float(best_row['val_roc_auc']):.4f} :: "
                    f"test_roc_auc={float(best_row['test_roc_auc']):.4f}"
                )
...
- Treat these as strong preliminary results from the current `{search_mode}` search, not as the final exhaustive tuning run.
- The leading combination is `{best_family} + {best_block}`, which suggests the best current performance comes from combining expert features, engineered features, local spatial context, and the selected autoencoder features.
...
def main() -> None:
    ...
    print("[part3] wrote candidate and outer-split tables")
    ...
    print("[part3] summarized model selection")
    ...
    print("[part3] generated family diagnostics")
